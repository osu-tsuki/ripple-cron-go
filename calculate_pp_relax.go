...
type ppRXUserMode struct {
	countScores int
	ppTotal     int
}

func opCalculateRXPP() {
	defer wg.Done()

	const ppQuery = "SELECT scores_relax.userid, pp, scores_relax.play_mode FROM scores_relax INNER JOIN users ON users.id=scores_relax.userid JOIN beatmaps USING(beatmap_md5) WHERE completed = 3 AND ranked >= 2 AND disable_pp = 0 AND pp IS NOT NULL ORDER BY pp DESC"
...
func roundRX(a float64) float64 {
	if a < 0 {
